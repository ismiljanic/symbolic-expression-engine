...
public class PredefinedMatrices {
    ...
    public static Symbol[][] getPredefinedTestMatrix4(Set<String> varsUsed) {
        ...
        matrix[0][0] = parseSymbolicInput("10*(x_l-3) + 6*(x_l+1) - lambda", varsUsed);
        matrix[0][1] = parseSymbolicInput("-6*(x_l)*(x_l+1) - d", varsUsed);
        matrix[0][2] = parseSymbolicInput("0", varsUsed);
        matrix[0][3] = parseSymbolicInput("0", varsUsed);

        matrix[1][0] = parseSymbolicInput("4", varsUsed);
        matrix[1][1] = parseSymbolicInput("10*(x_l-3)-(x_l) - lambda", varsUsed);
        matrix[1][2] = parseSymbolicInput("-3*((x_l-1)*(x_l) - d)", varsUsed);
        matrix[1][3] = parseSymbolicInput("0", varsUsed);

        matrix[2][0] = parseSymbolicInput("0", varsUsed);
        matrix[2][1] = parseSymbolicInput("7", varsUsed);
        matrix[2][2] = parseSymbolicInput("10*(x_l-3) - 6*(x_l-1) - lambda", varsUsed);
        matrix[2][3] = parseSymbolicInput("-1*((x_l-2)*(x_l-1) - d) ", varsUsed);

        matrix[3][0] = parseSymbolicInput("0", varsUsed);
        matrix[3][1] = parseSymbolicInput("0", varsUsed);
        matrix[3][2] = parseSymbolicInput("9", varsUsed);
        matrix[3][3] = parseSymbolicInput("10*(x_l-3) - 9*(x_l-2) - lambda", varsUsed);
        return matrix;
    }
    ...
    public static Symbol[][] getPredefinedTestMatrix5(Set<String> varsUsed) {
        ...
        matrix[1][0] = parseSymbolicInput("-4", varsUsed);
        matrix[1][1] = parseSymbolicInput("10*(x_l-3) - 1*(x_l) - lambda", varsUsed);
        ...
        matrix[4][4] = parseSymbolicInput("(x_l-2)*(x_l-1)*x_l*(x_l+1)+(((5*(x_l+1))+(3*(x_l-3)))*((x_l-2)+x_l)+(6*(x_l-1)*x_l)-2*(x_l-3)*(x_l-2)+2*d)*(d/10) - lambda", varsUsed);
