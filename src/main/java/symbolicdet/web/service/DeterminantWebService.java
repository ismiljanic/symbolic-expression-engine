...
public class DeterminantWebService {
    ...
    private DeterminantResponse doCompute(DeterminantRequest req) {
        ...
        for (int i = 0; i < n && allNumeric; i++)
            for (int j = 0; j < n && allNumeric; j++)
                if (!CheckIfNumeric.isNumeric(data[i][j]))
                    allNumeric = false;
        ...
        if (!allNumeric) {
            ...
            if (req.isDetailedLU()) response.setLuSteps(tracer.getLog());

            if (req.isFactorize()) {
                // Get the raw factorized data and convert to FactorizedGroup list
                List<FactorizedGroup> factorizedGroups = FactorizeFinalExpression.factor(expandedPoly, n);
                response.setFactorized(factorizedGroups);
            }
            ...
            if (req.hasNumericInputs()) {
                try {
                    ...
                    BigDecimal[][] numericData = SubstituteNumericMatrix.substituteNumericMatrix(matrix, variables);
                    SymbolicTracer luTracer = new SymbolicTracer(req.isDetailedLU());
                    BigDecimal numericDetLU = tracedNumericDeterminant(numericData, luTracer, req.isDetailedLU());
                    ...
                    response.setError("Symbolic result computed but numeric evaluation failed: " + e.getMessage());
                }
            }
        ...
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++)
                numericData[i][j] = BigDecimal.valueOf(data[i][j].eval(new HashMap<>()));

        SymbolicTracer tracer = new SymbolicTracer(req.isDetailedLU());
        BigDecimal numericDet = tracedNumericDeterminant(numericData, tracer, req.isDetailedLU());
        response.setNumericFromLU(numericDet.doubleValue());
        if (req.isDetailedLU()) response.setLuSteps(tracer.getLog());
    ...
    private Map<String, Double> buildVariableMap(
        ...
        for (String varName : tLVars) {
            ...
            double xLp1  = x - 2.0 * (lShifted + 1.0 - 1.0) * (lShifted + 1.0);
            vars.put(varName, xL * xLp1 - d);
        }

        for (String varName : varsUsed) {
            if (!vars.containsKey(varName)) {
                vars.put(varName, 0.0);
            }
        }
