...
public class PredefinedMatrices {
    ...
    public static Symbol[][] getPredefinedTestMatrix4(Set<String> varsUsed) {
        ...
        matrix[0][0] = parseSymbolicInput("6*(x_l-2)+3*(x_l+1)", varsUsed);
        ...
        matrix[1][1] = parseSymbolicInput("6*(x_l-2)-2*x_l", varsUsed);
        ...
        matrix[2][2] = parseSymbolicInput("6*(x_l-2)-5*(x_l-1)", varsUsed);
        ...
        matrix[3][3] = parseSymbolicInput("(x_l-1)*x_l*(x_l+1)+((x_l-1)+(x_l+1))*(d/2)", varsUsed);
        return matrix;
    }
